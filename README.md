...
### 個股詳細資訊
- 顯示個股基本資料（公司名稱、產業別、資本額等）
- 展示個股財務數據（EPS、ROE、ROA 等）
- 即時股價與成交量資訊
- 完整的技術指標與走勢圖表
...
### 智慧查詢
- 當查詢不存在的股票代號時，自動檢查是否為 ETF
- 若為 ETF 則自動跳轉至 ETF 查詢頁面並顯示相關資訊
- 提供流暢的使用者體驗，減少手動操作步驟
...
#### 取得個股財務資料
- **端點**: `GET /api/stock-finance/:stockId`
- **說明**: 獲取指定股票代號的財務資料
- **參數**:
  - `stockId` (路徑參數): 股票代號
- **快取**: 5 分鐘
- **範例回應**:
  ```json
  {
    ...
    "data": {
      ...
      "ROA": "21.50%",
      "grossMargin": "56.30%",
      "operatingMargin": "45.60%",
      "netProfitMargin": "38.90%",
      "debtRatio": "34.20%",
      "currentRatio": "1.78",
      "quickRatio": "1.52",
...
- **範例回應**:
  ```json
  {
    ...
    "lastUpdated": "2025-06-19T15:30:00.000Z",
    "fromCache": false
  }
  ```
...
### 歷史交易資料
- 來源：臺灣證券交易所 STOCK_DAY_ALL API
- 端點：`https://www.twse.com.tw/exchangeReport/STOCK_DAY_ALL`
- 更新頻率：每日收盤後更新
- 資料內容：
  - 每日收盤價
  - 成交量
...
## 資料快取機制
- 所有 API 端點都實作了快取機制，預設快取時間為 5 分鐘
- 快取資料會在本機記憶體中保存，伺服器重啟後會重新取得
- 每個 API 回應都包含 `lastUpdated` 和 `fromCache` 欄位，標示資料更新時間和是否來自快取

## 錯誤處理
- 所有 API 端點都會返回標準化的 JSON 回應
- 錯誤回應格式：
  ```json
  {
    "success": false,
...
#### 變更
- 移除了產業代號 API 對台灣證券交易所的外部依賴
- 改為使用內建的產業代號對照表
- 在 API 回應中添加了 `isSampleData: true` 標記
- 提高了系統的穩定性和響應速度

#### 修復
- 修正了因外部 API 變更導致的產業代號查詢失敗問題
- 優化了錯誤處理機制
